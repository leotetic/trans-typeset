...
v2 在该 contract 之上新增 workflow 智能排版骨架。`InputSource`、`AssetIR`、`UserIntent@0.2`、`WorkflowRun`、`SemanticLayoutAnalysis@0.2` 和 `LayoutIntentPlan@0.2` 用于记录输入归一化、用户意图、agent 语义识别、语义计划、步骤状态和诊断。`SemanticLayoutAnalysis` 和 `LayoutIntentPlan` 都只表达语义信号，不含 `bbox`、`x`、`y`、`page`、`width`、`height` 等坐标或分页字段。旧的 `schema_version="0.1"` payload 仍可 validate；新增字段都有默认值。

## v2 Workflow Contract

- `InputSource`: 记录 text/image/pdf/docx 输入类型、`source_role`、文件名、MIME、大小、hash、本地 artifact path 和诊断 flags。PDF workflow 使用 `content` 表示待翻译内容源，使用 `layout_reference` 表示可选版式语义参考源；未提供版式参考时写入 `layout_source_fallback_to_content`。DOCX workflow 先通过 headless LibreOffice/`soffice` 转为 PDF，再复用 PDF parser 保留 Word 渲染后的页面几何。
- `AssetIR`: 记录图片或参考资产、OCR/mock 摘要、alt text、来源 block 和不确定性 flags。
- `UserIntent`: 记录 `target_lang`、`workflow_mode`、`output_kind`、`style_intent`、`typesetting_standard`、自然语言排版说明、保留策略、约束、`column_layout`、`task_intent`、`output_targets`、`template_profile`、`bibliography_preference` 和 `requirements`。`workflow_mode=translate_only|typeset_only|translate_and_typeset` 是用户功能入口；`typeset_only` 会跳过 translator 并保留源文本。后端会把包含 `GB/T 7713.1` 的说明归一化为 `gb_t_7713_1_2025`，并把 `双栏`、`两栏`、`double column`、`two-column` 等说明归一化为 `column_count=2`。
- `WorkflowRun`: 记录 workflow 状态、当前 step、进度、输入源、用户意图、artifact refs、错误和每个 `WorkflowStep`。
- `SemanticLayoutAnalysis`: 记录 block role candidates、section hints、asset usage hints、结构候选、block-to-section mapping、`recognized_requirements`、缺失/不确定 section、confidence 和质量 flags，是 debug/agent artifact，不直接传给 renderer。
- `LayoutIntentPlan`: 记录语义排版计划，不含坐标。除 `blocks/assets/column_layout` 外，还包含 `document_profile`、`structure_plan`、`page_setup`、`style_system`、`numbering_plan`、`bibliography_plan` 和 `requirements`。renderer 当前只消费安全子集；最终坐标、分页、分栏流动、溢出和续页仍由 renderer 决定。
- `EditScope`: 历史重排的编辑范围，`mode=all|pages|blocks`；`pages` 使用 1-based `page_numbers`，`blocks` 使用 `DocumentIR` 中的稳定 `block_ids`。

当前 agent loop 由 LangGraph `StateGraph` 编排为 `read_input -> analyze_intent -> semantic_recognize -> build_plan -> validate_plan -> translate/source-preserve -> render -> evaluate_render -> optional repair -> export_pdf -> complete`。`workflow_mode=typeset_only` 或 legacy `output_kind=typeset_document` 会跳过真实 translator，生成 `translated_text == source_text` 的 renderer-compatible plans，并写入 `translation_skipped` / `source_text_preserved` flags。`workflow_mode=translate_only` 仍输出 preview/PDF，但跳过模型增强的智能排版计划，只使用确定性最小 layout plan。未安装或未配置模型 provider 时，后端使用同一批节点的 deterministic fallback；配置 OpenAI-compatible provider 后，LangChain structured output 可增强 `UserIntent`、`SemanticLayoutAnalysis` 和 `LayoutIntentPlan`，但所有模型输出仍必须通过 Pydantic/schema 校验。当用户说明包含 `GB/T 7713.1` 时，plan 会写入 `gb_t_7713_1_requested`，`UserIntent.typesetting_standard` 会设为 `gb_t_7713_1_2025`，renderer defaults 会切到 `continuous_reflow`。标题/小节倾向 `emphasis`，参考文献和脚注倾向 `compact`；这些仍是语义意图，LLM 不返回坐标。

## v0.2 Academic Intent Contract

- `TaskIntent`: 归一化 `course_paper`、`undergraduate_thesis`、`lab_report`、`proposal_report`、`book_report`、`social_practice_report`、`group_assignment`、`homework` 和 `generic_academic`，并保留 confidence/evidence。
- `AcademicRequirement`: 归一化自然语言里的结构、样式、metadata、编号、参考文献、篇幅、语气和资产要求。每条 requirement 有稳定 `requirement_id`、`category`、可选 `section_kinds`、evidence 和 quality flags；semantic/layout/renderer artifact 会把它标记为 `requirement_satisfied`、`requirement_diagnostic` 或 `requirement_recognized`。
- `OutputTarget`: 默认写入 `html_preview + pdf`；当自然语言明确要求 Word/DOCX 时写入 `docx` 目标，但当前后端仍只导出 PDF。
- `TemplateProfile`: 记录 `school_template`、`course_requirement`、`user_specified` 或 `default_academic`，以及 `standard` 和 fallback 状态。
- `DocumentStructurePlan`: 记录 `cover/title/abstract/keywords/toc/list_of_figures/list_of_tables/body/heading/figure/table/formula/references/appendix/acknowledgements` 以及实验报告 section，如 `experiment_purpose`、`experiment_theory`、`experiment_steps`、`experiment_results`、`experiment_analysis`、`result_analysis`。
- `NumberingPlan` 与 `BibliographyPlan`: 记录 heading/figure/table/formula/reference numbering、目录生成和 `auto/gb_t_7714/apa/mla/ieee/chicago` 引用格式偏好。校验会拒绝引用不存在的 `section_id`。
...
- `column_layout`: 默认 `column_count=1`、`column_gap_pt=18.0`、`scope=body`、`balance_columns=false`。当前 MVP 支持一栏和双栏；`balance_columns` 仅作为 contract 预留。`scope=body` 时 title、abstract、major heading、figure/table/formula 等结构性内容保持全宽，正文按栏流动。
...
`layout_mode=continuous_reflow` 是默认 GB/T 7713.1 中文可读重排：renderer 按全局阅读顺序生成连续 A4 页面和确定性 bbox，跳过竖排时间戳、重复页脚、无 path 的 vector placeholder，对长段做句子/词边界分页，并输出居中阿拉伯页码。当 `RenderDefaults.column_layout.column_count=2` 时，renderer 根据页边距和 `column_gap_pt` 计算两栏正文宽度，正文先流入第一栏再流入第二栏；标题、摘要、主要 heading、图表和公式保持全宽。`layout_mode=source_bbox` 可用于保留原 PDF 页面尺寸和 bbox，但字体、字号、行高、对齐和缩进仍应优先遵循 `RenderDefaults.role_styles`，再按缩放、扩盒、续页实现 overflow policy。
...
导出文件包含 v2 workflow contract、semantic/layout intent contract、`EditScope`、`DocumentIR`、`TranslationChunk`
...
- `docx-conversion`: DOCX 输入的 LibreOffice 转 PDF 诊断，包含 converter、源路径、转换后 PDF 路径、输出大小和失败原因。
...
- `layout-trace`: renderer 的逐块排版决策日志，包括 layout mode、标准、render defaults snapshot、`column_layout`、源页/输出页统计、跳过块/资产、页利用率、每个 source block 的分页片段、bbox、估算行数、`column_count`、`column_index`、`span` 和质量 flags。
- `renderer-diagnostics`: renderer 根据 `DocumentIR + TranslationLayoutPlan` 生成的质量诊断，包括缺失译文、角色不一致、溢出 flags、页利用率、低利用率页、单片段页、被跳过 artifact 摘要和 `intent_requirements`。`intent_requirements` 会把每个自然语言要求标记为 `satisfied`、`diagnostic` 或 `recognized`，因此缺封面、目录、图表目录、实验报告 section、课程 metadata 等都会在本地 UI inspector 中可见。
